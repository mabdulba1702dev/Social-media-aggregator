...
- **Telegram: fully working end-to-end, 2026-08-27 — the webhook mismatch is fixed for real.** Root cause found via `vercel env pull` compared byte-for-byte against `.env.local` (not guessed): both `TELEGRAM_WEBHOOK_SECRET` (64 chars locally vs. 13 in Vercel) and `TELEGRAM_BOT_TOKEN` had drifted — rotated locally at some point, never pushed to Vercel. The `-baqidev`/Preview-vs-Production theory from the earlier session was a red herring — `vercel alias ls` confirmed the registered webhook URL (`social-media-aggregator-ten.vercel.app`) already resolved to the real Production deployment. Fixed via `vercel env rm`/`vercel env add` (Production + Preview) to match the local, Telegram-confirmed-working values, then `vercel deploy --prod` to pick them up. Verified for real: a direct `curl` with the correct `X-Telegram-Bot-Api-Secret-Token` now gets `200` (wrong token still correctly `401`), and the 3 messages that had queued up in Telegram's retry buffer during the outage actually drained into the database — `ingestion_events` shows real `processed`/`duplicate` rows against the `f5e21444…` "Testing group" source, and `sources.last_event_at` (see the fix above) is now populated for real, not `null`. This is the first fully-proven Telegram ingestion, end to end.
...
- **Telegram webhook secret mismatch** — fixed and verified end-to-end 2026-08-27, see "What's working" above.
...
- **`sources.last_event_at` fix** — fixed 2026-08-27 and re-verified against real Telegram traffic in the same session (see "What's working") — `last_event_at` is now populated for the "Testing group" source. Resolved.
...
- 2026-08-27 — Fixed the real Telegram webhook 401 (`TELEGRAM_WEBHOOK_SECRET`/`TELEGRAM_BOT_TOKEN` had drifted between `.env.local` and Vercel) via `vercel env rm`/`add` + `vercel deploy --prod`. Verified end-to-end against real Telegram traffic — see "What's working." **Note on process:** this went out via a direct `vercel deploy --prod` CLI deploy rather than the usual PR-merge-to-`main` flow, since the fix was an env var change with no code to review — production is now briefly ahead of `main` (it's serving `dev`'s tip, which includes PR #32's `last_event_at` fix). Logging this per `CLAUDE.md`'s rule rather than leaving it implicit; `dev` should get promoted to `main` in the normal way to reconcile. Local `vercel link` also ran for the first time this session (added `.vercel/` — already gitignored — and refreshed `.env.local`'s `VERCEL_OIDC_TOKEN`, confirmed still gitignored).
