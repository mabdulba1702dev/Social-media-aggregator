...
No browser/computer-use tool is available to me in this session — confirmed via direct investigation (Vercel's deployment protection blocks plain HTTP fetches against every project URL; a Playwright MCP server was configured but never bound to this specific session despite the browser binary being installed and the server process confirmed running, re-checked 2026-08-23, still not bound). **Your screenshots are the actual working substitute** and have caught every real UI bug found so far (duplicate boards, invisible navigation, the Twitter width overflow, the lazy-mount hydration bug, YouTube's tiny embed size) — this isn't a blocker, just the established workflow.

### To do — real testing without a browser (2026-08-23, not yet built)

Two separate problems, two separate plans:
1. **Behavioral/data-layer testing** (does the right DB row get created, does dedup actually reject, does RLS actually block) — doesn't need a browser or Google at all. Plan: use Supabase's admin API to mint a real session for a disposable test user without touching Google's login screen — `admin.generateLink({ type: 'magiclink', email })` returns a token, exchanged via `verifyOtp()` for a real access token, then build a Supabase client scoped to that token (RLS-enforced like a real signed-in user, **not** the service-role client, which would mask real RLS bugs the same way the "delete doesn't work" investigation flagged). Deliberately doesn't try to automate Google's actual OAuth consent screen — that's fragile and likely to trip bot detection for any provider, not a limitation specific to this environment; real E2E suites mint sessions directly instead of scripting the third-party login form.
2. **Visual verification** (does the modal actually render right, is a layout broken) — genuinely needs a browser. Worth trying the `run` skill once, which might drive a browser through a different path than the Playwright MCP tool that keeps failing to bind — not yet tried this session. Until/unless that works, real screenshots stay the primary way UI bugs get caught here.
