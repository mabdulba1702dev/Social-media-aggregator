...
6a. [x] **Sources UI** (`app/boards/[boardId]/sources/`) — connect/rename/pause/disconnect a Telegram/Discord/WhatsApp group against a board, replacing the "insert a `sources` row directly" workaround `setup-guide.md` previously documented. v1 still requires the user to paste in the platform's raw group/channel ID by hand (Telegram chat ID, Discord channel ID, WhatsApp JID) — there's no in-app discovery mechanism for that ID yet (would need a "detected but unconnected" staging table reading from `ingestion_events`, not built). In-form guidance text explains how to find each platform's ID manually, mirroring what `setup-guide.md` §6 already documented. `unique(platform, external_group_id)` conflicts surface as "already connected to a board," not a raw DB error.
