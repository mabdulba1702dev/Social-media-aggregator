...
6. [x] **Masonry board view** — basic CSS-columns grid built (`app/boards/[boardId]/page.tsx`), renders real embeds. Embeds now lazy-mount via `IntersectionObserver` (`components/lazy-mount.tsx`) instead of all rendering immediately — each oEmbed `<script>` only fetches/executes once its card scrolls near the viewport.
...
4. [x] **`worker/` process stood up locally** — npm workspace package, both WhatsApp (`worker/src/whatsapp.ts`) and Discord (`worker/src/discord.ts`) sides built. Not yet deployed to Railway/Fly.io — see item 5.
5. [x] **Discord bot (Gateway)** — `discord.js` (chosen — the natural default, wasn't pinned anywhere before this), same `handleIncomingMessage` pipeline entry point as WhatsApp/Telegram, keyed on channel ID (matching the sources UI's "Copy Channel ID" guidance) not the whole server. Untested against a real server/bot — needs `DISCORD_BOT_TOKEN` + Message Content Intent enabled in the Discord Developer Portal, and the worker isn't deployed anywhere (see item 4).
6. [x] **WhatsApp (Baileys) — code built**, not yet live: connection + pairing-code auth + session persistence + pipeline wiring done (`worker/src/whatsapp.ts`, `worker/src/pipeline.ts`). The sources UI (item 6a below) now covers connecting a group to a board. Still blocked on: a dedicated non-primary number to pair against for real (risk-mitigation checklist from `docs/setup-guide.md` §6 still applies before going live), and the worker itself isn't deployed anywhere yet — see item 4.
6a. [x] **Sources UI** (`app/boards/[boardId]/sources/`) — connect/rename/pause/disconnect a Telegram/Discord/WhatsApp group against a board, replacing the "insert a `sources` row directly" workaround `setup-guide.md` previously documented. Connecting is a 3-step modal (`connect-source-modal.tsx`, using a new `components/ui/dialog.tsx` primitive — the app's first): pick platform → per-platform instructions + paste the raw ID → confirmation. v1 still requires the user to paste in that raw ID by hand (Telegram chat ID, Discord channel ID, WhatsApp JID) — there's no in-app discovery mechanism for it yet (would need a "detected but unconnected" staging table reading from `ingestion_events`, not built), and deliberately drops the mockup's literal "QR code" step (no real scan-to-join flow exists for any of these three platforms) — see `notes/ui-scalability-scope.md` for the future-QR want, logged not built. `unique(platform, external_group_id)` conflicts surface as "already connected to a board," not a raw DB error.
